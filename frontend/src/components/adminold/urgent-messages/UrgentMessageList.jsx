    ...
    try {
      ...
      setMessages(prevMessages => 
        prevMessages.map(msg => {
          ...
          // If activating a message, ensure all others are inactive
          if (newStatus === 'active' && msg.status === 'active') {
    ...
    return (
      <div className="admin-layout">
        ...
        <div className="admin-content">
          <div className="loading-message">Loading urgent messages...</div>
        </div>
      </div>
    );
  }
              ...
              {messages.map((message) => (
                <tr key={message.id} className={message.status === 'inactive' ? 'inactive' : ''}>
                  ...
                  <td className="image-cell">
                    {message.image_path ? (
                      <img 
                        src={`${message.image_path.startsWith('http') ? '' : (import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000')}${message.image_path.startsWith('/') ? '' : '/'}${message.image_path}`} 
                        alt={message.title} 
