...
const AddUrgentMessage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (image) {
        try {
          const uploadResponse = await apiService.upload.image(image);
          imagePath = uploadResponse.path; // Adjust this based on your API response
        } catch (uploadError) {
      ...
      if (formData.activate_immediately) {
        try {
          const activeMessages = await apiService.urgentMessages.getAll({ status: 'active' });
          // Check if activeMessages is an array before mapping
          if (Array.isArray(activeMessages)) {
            await Promise.all(
              activeMessages.map(msg => 
                apiService.urgentMessages.updateStatus(msg.id, 'inactive')
              )
            );
          }
        } catch (deactivateError) {
          console.error('Error deactivating messages:', deactivateError);
          throw new Error('Failed to deactivate other messages. Please try again.');
        }
      }
      ...
      const messageData = {
        ...
        action_text: formData.action_text || 'urgent'
      };
      ...
      await apiService.urgentMessages.create(newMessage);
